...
def build_embedding_input(payload: Dict[str, Any]) -> str:
    ...
    Concatenate title, category, tagline, author, and description (fallback to content).
    """
    title = payload.get("title")
    category = payload.get("category")
    tagline = payload.get("tagline")
    author = payload.get("author")
    # Prefer 'description' for embedding; handle misspelling 'decription', fallback to 'content'.
    description = payload.get("description") or payload.get("decription")
    if description is None:
        description = payload.get("content")
    return safe_join_text([title, category, tagline, author, description])


def process_file(gclient: genai.Client, path: str, force: bool = False) -> Optional[Tuple[str, bool]]:
    try:
    ...
    if not force and isinstance(data, dict) and "embedding" in data:
        # Return (path, updated=False) to indicate a skip
        return (path, False)
    ...
    # Return (path, updated=True) to indicate we wrote embedding
    return (path, True)
...
def main(argv: Optional[List[str]] = None) -> int:
    ...
    for p in targets:
        res = process_file(gclient, p, force=args.force)
        if res is None:
            failures += 1
            continue
        path, updated = res
        if updated:
            ...
            skipped += 1
            print(f"Skipped (embedding exists): {path}")
